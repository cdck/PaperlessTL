...
class AvcEncoder {
    ...
    public AvcEncoder(int width, int height, int framerate, int bitrate, int imageFormat) throws IOException {
        LogUtils.d(TAG, "AvcEncoder -->width= " + width + ",height= " + height + ",framerate= " + framerate + ",bitrate= " + bitrate + ",imageFormat= " + imageFormat);
        ...
        LogUtils.d(TAG, "encoder mColorFormat=" + mColorFormat);
...
//        }
    ...
    public void startEncoderThread() {
        LogUtils.d(TAG, "startEncoderThread -->");
                ...
                if (input != null) {
                    try {
                        ...
                        if (inputBufferIndex >= 0) {
                            ...
                            //LogUtils.v(TAG, "startEncoderThread:  queueInputBuffer时间戳= " + presentationTimeUs);
...
//        encoderThread.start();
    ...
    private void sendDatas() {
        ...
        while (index >= 0) {
            LogUtils.v(TAG, " sendDatas Get H264 Buffer Success! flag = " + bufferInfo.flags + ",pts = " + bufferInfo.presentationTimeUs);
    ...
    private void timePush(byte[] data, boolean is_key_frame, long presentationTimeUs) {
        ...
        if (useTime <= hm) {
            try {
                ...
                LogUtils.v(TAG, "timePush-> 睡眠：" + millis + ",当前时间：" + System.currentTimeMillis());
                Thread.sleep(millis);
                LogUtils.v(TAG, "timePush-> 睡眠：" + millis + ",当前时间：" + System.currentTimeMillis());
            ...
            LogUtils.v(TAG, "timePush-> 直接发送出去 hm= " + hm + ", useTime= " + useTime);
            lastTime = System.currentTimeMillis();
            jni.call(channelIndex, iskeyframe, presentationTimeUs, data);
        }
    }
