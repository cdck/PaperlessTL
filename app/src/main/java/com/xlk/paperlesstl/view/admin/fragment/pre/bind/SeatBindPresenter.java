package com.xlk.paperlesstl.view.admin.fragment.pre.bind;
...
//                queryPlaceRanking();
    ...
    public void queryPlaceRanking() {
        ...
        if (info != null) {
            ...
            for (int j = 0; j < seatData.size(); j++) {
                ...
                LogUtils.i(TAG, "queryPlaceRanking devName=" + item.getDevname().toStringUtf8() + ",devId=" + item.getDevid()
                ...
                for (int i = 0; i < devSeatInfos.size(); i++) {
                    ...
                    if (item.getMemberid() == bean.getMember().getPersonid()) {
                        LogUtils.d(TAG, "queryPlaceRanking " + bean.getMember().getName().toStringUtf8());
                        bean.setSeat(item);
                        break;
                    }
                }
    ...
    public void bindSeat(List<ReadJxlBean> readJxlBeans) {
        LogUtils.i(TAG, "bindSeat readJxlBeans.size=" + readJxlBeans.size());
        for (int i = 0; i < readJxlBeans.size(); i++) {
            ...
            LogUtils.i(TAG, "bindSeat memberName=" + memberName + ",devId=" + devId + ",devName=" + devName);
            int memberId = 0;
