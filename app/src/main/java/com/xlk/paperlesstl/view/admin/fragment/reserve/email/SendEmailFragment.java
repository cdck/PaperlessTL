...
import com.xlk.paperlesstl.helper.EmailThread;
...
public class SendEmailFragment extends BaseFragment implements SendEmailInterface, View.OnClickListener {
    ...
    private void updateUI(ArrayList<String> selectedMember) {
        ...
        if (isFilter) {
            ToastUtils.showShort(R.string.already_filtered_email);
        }
    }
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_send_email: {
                ...
                if (!account.isEmpty() && RegexUtils.isEmail(email)) {
                    ...
                    ToastUtils.showShort(R.string.please_enter_account);
                    return;
                }
                if (!pwd.isEmpty()) {
                ...
                if (theme.isEmpty() || content.isEmpty()) {
                    ToastUtils.showShort(R.string.please_enter_theme_content);
                    return;
                }
                String trim = edt_recipient.getText().toString().trim();
                if (trim.isEmpty()) {
                    ToastUtils.showShort(R.string.please_enter_receive_email);
                    return;
                }
                ...
                for (int i = 0; i < strings.size(); i++) {
                    ...
                    if (!RegexUtils.isEmail(s)) {
                        ToastUtils.showShort(R.string.err_receive_email_format);
                        return;
                    }
                }
