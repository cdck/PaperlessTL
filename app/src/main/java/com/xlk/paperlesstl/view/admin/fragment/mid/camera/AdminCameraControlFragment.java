...
import com.xlk.paperlesstl.view.admin.adapter.MeetLiveVideoAdapter;
...
public class AdminCameraControlFragment extends BaseFragment implements AdminCameraControlInterface, View.OnClickListener, ViewClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        ids.add(Constant.RESOURCE_ID_1);
        ids.add(Constant.RESOURCE_ID_2);
        ids.add(Constant.RESOURCE_ID_3);
        ids.add(Constant.RESOURCE_ID_4);
        initAdapter();
        custom_video_view.post(() -> {
...
//                    }
    ...
    private void showProPop(boolean isStart, VideoDev videoDev) {
        ...
        proPop = PopupUtil.createHalfPop(inflate, btn_start_pro);
    ...
    private void proHolderEvent(CustomBaseViewHolder.ProViewHolder holder, boolean isStart, VideoDev videoDev) {
        ...
        holder.wm_pro_launch_pro.setOnClickListener(v -> {
            ...
            if (checked) {
                res.add(Constant.RESOURCE_ID_0);
            } else {
                if (holder.wm_pro_flow1.isChecked()) res.add(Constant.RESOURCE_ID_1);
                if (holder.wm_pro_flow2.isChecked()) res.add(Constant.RESOURCE_ID_2);
                if (holder.wm_pro_flow3.isChecked()) res.add(Constant.RESOURCE_ID_3);
                if (holder.wm_pro_flow4.isChecked()) res.add(Constant.RESOURCE_ID_4);
            }
            if (res.isEmpty()) {
            ...
            if (isStart) {//发起投影
                ...
                JniHelper.getInstance().streamPlay(deviceid, subid, triggeruserval, res, ids);
            } else {//结束投影
                JniHelper.getInstance().stopResourceOperate(res, ids);
            }
            proPop.dismiss();
        });
    }
        ...
        holder.wm_screen_launch.setOnClickListener(v -> {
            ...
            if (ids.isEmpty()) {
                ...
                if (isStart) {//发起同屏
                    ...
                    JniHelper.getInstance().streamPlay(deviceid, subid, triggeruserval, temps, ids);
                } else {//结束同屏
                    JniHelper.getInstance().stopResourceOperate(temps, ids);
                }
                screenPop.dismiss();
            }
        });
            ...
            adapter.setOnItemClickListener(new OnItemClickListener() {
                ...
                public void onItemClick(@NonNull BaseQuickAdapter<?, ?> ad, @NonNull View view, int position) {
                    if (videoDevs.get(position).getDeviceDetailInfo().getNetstate() == 1) {
                        ...
                        LogUtils.d(TAG, "onItemClick --> Subid= " + videoDetailInfo.getSubid());
