...
import static com.xlk.paperlesstl.model.Constant.isThisDevType;
...
//                int mikeId = Integer.parseInt(currentMikeId);
            ...
            case R.id.btn_delete:
                ...
                if (selectedDevice.getNetstate() == 0) {
                    ...
                    ToastUtils.showShort(R.string.err_delete_offline);
                }
                break;
            //设置访客模式
            case R.id.btn_visitors: {
                ...
                if (isThisDevType(InterfaceMacro.Pb_DeviceIDType.Pb_DeviceIDType_MeetClient_VALUE, devcieid)) {
                    ...
                    ToastUtils.showShort(R.string.please_choose_client_dev);
                }
                break;
            }
            //参数配置
    ...
    private void showParamConfigPop() {
        ...
        paramPop = PopupUtil.createCenter(inflate, GlobalValue.screen_width - 100, GlobalValue.screen_height - 100,btn_deploy);
    ...
    private void HolderEvent(ViewHolder holder) {
        ...
        holder.btn_modify.setOnClickListener(v -> {
            ...
            if (holder.cb_ip.isChecked()) {
                if (ip.isEmpty()) {
                    ToastUtils.showShort(R.string.please_enter_ip);
                    return;
                }
                itemStr += "{"
                        + "\"section\":\"areaaddr\","
            ...
            if (holder.cb_port.isChecked()) {
                if (port.isEmpty()) {
                    ToastUtils.showShort(R.string.please_enter_port);
                    return;
                }
                if (!itemStr.isEmpty()) {
                    itemStr += ",";
            ...
            if (holder.cb_cache_dir.isChecked()) {
                if (cache_dir.isEmpty()) {
                    ToastUtils.showShort(R.string.please_enter_cache_dir);
                    return;
                }
                if (!itemStr.isEmpty()) {
                    itemStr += ",";
            ...
            if (holder.cb_cache_size.isChecked()) {
                if (cache_size.isEmpty()) {
                    ToastUtils.showShort(R.string.please_enter_cache_size);
                    return;
                }
                if (!itemStr.isEmpty()) {
                    itemStr += ",";
    ...
    private void showChooseDir(EditText edt) {
        ...
        PopupWindow dirPop = PopupUtil.createHalfPop(inflate,btn_deploy);
        ...
        if (deviceAdapter == null) {
            ...
            deviceAdapter.setOnItemClickListener(new OnItemClickListener() {
                ...
                public void onItemClick(@NonNull BaseQuickAdapter<?, ?> adapter, @NonNull View view, int position) {
                    ...
                    LogUtils.i(TAG, "onItemClick 选中设备= id：" + selectedDevice.getDevcieid() + ",名称：" + selectedDevice.getDevname().toStringUtf8());
