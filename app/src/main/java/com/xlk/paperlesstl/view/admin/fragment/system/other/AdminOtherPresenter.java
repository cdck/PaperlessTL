...
public class AdminOtherPresenter extends BasePresenter {
    ...
    protected void busEvent(EventMessage msg) throws InvalidProtocolBufferException {
        switch (msg.getType()) {
            ...
            case EventType.BUS_NOTICE_BG: {
                String filePath = (String) msg.getObjects()[0];
                view.get().updateNoticeBgImg(filePath);
                break;
            }
            case EventType.BUS_NOTICE_LOGO: {
                String filePath = (String) msg.getObjects()[0];
            ...
            case InterfaceMacro.Pb_Type.Pb_TYPE_MEET_INTERFACE_DEFAULTURL_VALUE: {
                if (msg.getMethod() == InterfaceMacro.Pb_Method.Pb_METHOD_MEET_INTERFACE_NOTIFY_VALUE) {
                    ...
                    LogUtils.i(TAG, "BusEvent 网页变更通知: isetdefault=" + isetdefault + ",size=" + itemList.size());
                    for (int i = 0; i < itemList.size(); i++) {
                        ...
                        int id = item.getId();
                        LogUtils.i(TAG, "BusEvent name=" + name + ",addr=" + addr + ",id=" + id);
                    }
                    webQuery();
                }
                break;
            ...
            case InterfaceMacro.Pb_Type.Pb_TYPE_MEET_INTERFACE_ADMIN_VALUE: {
                ...
                int id = info.getId();
                int opermethod = info.getOpermethod();
                LogUtils.i(TAG, "BusEvent -->" + "管理员变更通知 id=" + id + ", opermethod=" + opermethod);
                queryAdmin();
                break;
            }
            default:
    ...
    public void queryInterFaceConfiguration() {
            ...
            if (info != null) {
                ...
                for (int i = 0; i < pictureList.size(); i++) {
                    ...
                    LogUtils.i(TAG, "queryInterFaceConfiguration pictureList faceid=" + faceid + ",mediaid=" + mediaid);
                    String userStr = "";
                    //主界面背景
...
//                }
            ...
            LogUtils.e(TAG, "queryInterFaceConfiguration mainInterfaceBeans.size=" + mainInterfaceBeans.size()
        ...
        if (pbui_typeAdminDetailInfo != null) {
            ...
            for (int i = 0; i < itemList.size(); i++) {
                ...
                if (item.getAdminid() == AdminActivity.currentAdminId) {
                    LogUtils.i(TAG, "queryAdmin 获取登录的管理员信息 " + item.getAdminid() + "," + item.getAdminname().toStringUtf8());
                    localAdminInfo = item;
                    return;
                }
            }
    ...
    public void modifyAdminPassword(String oldPwd, String newPwd) {
        if (localAdminInfo == null) {
            LogUtils.e(TAG, "modifyAdminPassword 没有查找到本机的管理员信息");
            return;
        }
        String newMd5Pwd = ConvertUtil.md5(newPwd);
        String oldMd5Pwd = ConvertUtil.md5(oldPwd);
        LogUtils.i(TAG, "modifyAdminPassword newMd5Pwd=" + newMd5Pwd + ",oldMd5Pwd=" + oldMd5Pwd);
    ...
    public void queryUpdateFile() {
        ...
        LogUtils.i(TAG, "queryUpdateFile updateFileData.size=" + updateFileData.size());
