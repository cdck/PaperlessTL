package com.xlk.paperlesstl.view.admin.fragment.pre.agenda;
...
public class AdminAgendaFragment extends BaseFragment implements AdminAgendaInterface, View.OnClickListener {
            ...
            case R.id.btn_save_agenda:
                if (cb_file.isChecked()) {
                    ...
                    if (TextUtils.isEmpty(content)) {
                        ToastUtils.showShort(R.string.please_enter_agenda_content);
                        return;
                    }
    ...
    private void showAgendaFilePop(List<InterfaceFile.pbui_Item_MeetDirFileDetailInfo> agendaFiles) {
        ...
        LogUtils.i(TAG, "showAgendaFilePop fragment的大小 width=" + width + ",height=" + height);
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            if (requestCode == REQUEST_CODE_AGENDA_TEXT) {
                LogUtils.i(TAG, "onActivityResult 导入议程内容的文件=" + filePath);
                if (filePath.endsWith(".txt")) {
                    showProgressBar(true);

                    FileUtil.readTxtFile(EventType.BUS_READ_AGENDA_TXT, filePath);
                } else {
                    ToastUtils.showShort(R.string.can_only_choose_txt_file);
                }
            } else if (requestCode == REQUEST_CODE_AGENDA_FILE) {
                LogUtils.i(TAG, "onActivityResult 上传的议程文件=" + filePath);
