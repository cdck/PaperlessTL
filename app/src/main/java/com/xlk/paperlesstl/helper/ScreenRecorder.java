...
//        this.height = height;
    ...
    public void run() {
        ...
        try {
            ...
            LogUtils.v(TAG, "created virtual display: " + display);
            // 录制虚拟屏幕
    ...
    private void prepareEncoder() throws IOException {
        ...
        LogUtils.i(TAG, "prepareEncoder 修改前录制使用宽高=" + width + "," + height);
...
//        format.setInteger(MediaFormat.KEY_BITRATE_MODE, MediaCodecInfo.EncoderCapabilities.BITRATE_MODE_CBR);
    ...
    private void recordVirtualDisplay() {
        LogUtils.v(TAG, "recordVirtualDisplay---------------------------");
        while (!quit.get()) {
            //从输出队列中取出编码操作之后的数据
            //输出流队列中取数据索引,返回已成功解码的输出缓冲区的索引
            int index = encoder.dequeueOutputBuffer(bufferInfo, TIMEOUT_US);
            while (index >= 0) {
                LogUtils.v(TAG, "Get H264 Buffer Success! flag = " + bufferInfo.flags + ", pts = " + bufferInfo.presentationTimeUs + "");
                ...
                if (bufferInfo.flags == MediaCodec.BUFFER_FLAG_CODEC_CONFIG) {
                    LogUtils.v(TAG, "get config byte!");
                    ...
                    LogUtils.v(TAG, "recordVirtualDisplay :   --> 发送关键帧数据 " + keyframe.length);
                    timePush(keyframe, true, bufferInfo.presentationTimeUs);
                } else {
                    LogUtils.v(TAG, "recordVirtualDisplay :  else 路线 --> 发送其它帧数据：" + outData.length);
